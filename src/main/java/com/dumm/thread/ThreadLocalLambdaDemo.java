...
/**
 * Created by dumm on 2019/3/5.
 */

/**
 * @ProjectName: giteeLearn
 * @Package: com.dumm.thread
 * @ClassName: ThreadLocalLambdaDemo
...
class SafeBank {
    ...
    private ThreadLocal<Integer> balance2 = ThreadLocal.withInitial(() -> 2000);
    /**
     * 存款
     *
     * @param money 存款金额
    ...
    public void deposit(int money) {
        ...
        System.out.println(threadName + " -> 当前账户余额为：" + this.balance2.get());
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            e.printStackTrace();
